...
def insert_images_fixed(content, images):
    ...
    for img in images:
        ...
        if img.position_type == 'before_first':
            ...
            print(f"图片: {img.position_type} -> 位置0")
        elif img.position_type == 'after_last':
            # 插入到最后面
            insert_points.append((len(paragraphs), img_html))
            print(f"图片: {img.position_type} -> 位置{len(paragraphs)}")
        else:
            # custom / before_paragraph / after_paragraph
            # position_value直接对应段落号（1-based），不需要减1！
            pos = img.position_value
            
            if img.position_mode == 'after' or img.position_type == 'after_paragraph':
                # 在第pos段之后插入，位置就是pos
                insert_pos = pos
                print(f"图片: position_value={pos}, after -> 位置{insert_pos}")
            else:
                # 在第pos段之前插入，位置是pos-1
                insert_pos = pos - 1
                print(f"图片: position_value={pos}, before -> 位置{insert_pos}")
    ...
    print(f"\n插入顺序（从后往前）: {[p[0] for p in insert_points]}")
...
with app.app_context():
    ...
    print(f"\n=== 期望位置 ===")
    print(f"图片1 (before_first): 文章开头 (段落1)")
    print(f"图片2 (position_value=2, after): 第2段之后")
    print(f"图片3 (position_value=5, before): 第5段之前")
