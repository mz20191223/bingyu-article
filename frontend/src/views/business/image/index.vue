    ...
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading">
        ...
        <el-table-column prop="products" label="关联产品">
          <template #default="{ row }">
            <el-tag v-for="product in row.products" :key="product.id" type="info" size="small">{{ product.name }}</el-tag>
          </template>
        </el-table-column>
...
const formRef = ref()
const form = reactive({ id: null, url: '', positionType: 'auto', positionValue: 1, productIds: [], status: 1 })
...
const handleEdit = (row) => { Object.assign(form, { id: row.id, url: row.url, positionType: row.positionType, positionValue: row.positionValue, productIds: row.products ? row.products.map(p => p.id) : [], status: row.status }); dialogTitle.value = '编辑图片'; dialogVisible.value = true }
